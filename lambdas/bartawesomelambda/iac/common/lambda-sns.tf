...
module "sns" {
  ...
  description    = "bart awesome lambda description"
  tags           = var.tags
}

resource "aws_sns_topic_subscription" "bart-awesome-lambda_lambda_target" {
  topic_arn = module.sns.sns_arn
  protocol  = "lambda"
  endpoint  = module.lambdaSNS.lambda_arn
}

resource "aws_lambda_permission" "allow_sns_bart-awesome-lambda" {
